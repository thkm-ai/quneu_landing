...
export default function VisionSection() {
  ...
  return (
    <section id="vision" ref={sectionRef} className="py-24 px-6 relative overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {visionItems.map((item, index) => (
            ...
            >
              <div className="text-center border border-red-500 rounded-lg p-8 hover:border-red-600 transition-colors duration-300 h-full bg-white/70 backdrop-blur-sm">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-slate-800 to-slate-900 text-white text-3xl mb-6 shadow-lg group-hover:scale-110 transition-transform duration-300 border border-red-500">
                  <item.icon size={32} className="text-white" />
                </div>

                <h3 className="text-2xl font-thin mb-4 tracking-wider text-slate-900 text-whisper uppercase">
                  {item.title}
                </h3>

                <div className="w-16 h-px bg-gradient-to-r from-transparent via-slate-400 to-transparent mx-auto mb-6"></div>

                <p className="text-slate-700 leading-relaxed font-light text-airy max-w-sm mx-auto">{item.content}</p>
              </div>
            </div>
          ))}
        </div>
